import os
import sys # Import sys for error handling if needed
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import yaml # For loading config
from typing import Dict, List, Tuple, Optional
# Assuming 'corpus' and 'utils' are in the same directory or accessible in PYTHONPATH
# If not, adjust the import path or ensure the modules are installed correctly
try:
    # Need AlternativeCMUDict for isinstance check later
...
# ... (MyWERDetails class definition remains the same) ...
class MyWERDetails(WERDetails):
    ...
    def __init__(self, location, skip_calculation=False, config=None):
        ...
        if espeakng:
            try:
                # Use getattr safely for nested attributes
        ...
        # --- End Call parent init ---


    # word_to_phoneme method remains the same...
    def word_to_phoneme(self, word: str, stress_cleaned: bool) -> List[str]:
        ...
        """
        is_oov = True # Assume OOV initially
        # Use getattr to safely access cmudict, as it might not be set if parent init failed
        cmudict_obj = getattr(self, 'cmudict', None)

        if cmudict_obj:
             if isinstance(cmudict_obj, AlternativeCMUDict):
                 # Assuming get_arpabet returns the original word if OOV, and formatted string otherwise
                 ...
                 is_oov = "{" not in lookup_result # Relying on '{' presence
             ...
             # Add other dictionary type checks if necessary
             else:
        ...
        if self.g2p:
            try:
                ...
                if cleaned_phonemes:
                    # print(f"DEBUG: espeak-ng OOV '{word}' -> {cleaned_phonemes}") # Optional debug
                    return cleaned_phonemes
                else:
                    # print(f"DEBUG: espeak-ng OOV '{word}' -> [] (empty result)") # Optional debug
            ...
            # print(f"DEBUG: OOV '{word}' -> [] (espeak-ng unavailable)") # Optional debug
            return []
    ...
    def arpabet_cleaner(self, phoneme_str: str, stress_remove: bool = False) -> List[str]:
        ...
        # Remove primary/secondary stress markers, handle potential underscores
        cleaned_str = phoneme_str.replace("ˈ", "").replace("ˌ", "").replace("_", " ")
        # Remove length markers if desired (ː,ˑ) - uncomment if needed
        # cleaned_str = cleaned_str.replace("ː", "").replace("ˑ", "")
        # Remove syllable boundaries if present (.) - uncomment if needed
        ...
        if stress_remove:
             # Example ARPABET stress removal (adjust if needed)
             phonemes = [p.rstrip('0123456789') for p in phonemes]

        # --- Optional: Convert IPA to ARPABET ---
        # If your analysis relies strictly on ARPABET symbols, you'd need a
        # mapping dictionary here to convert IPA symbols from espeak-ng
        # to your expected ARPABET set. This can be complex.
        # Example (very basic, needs expansion):
        # ipa_to_arpabet_map = {'ə': 'AH', 'ɪ': 'IH', ...}
        # arpabet_phonemes = [ipa_to_arpabet_map.get(p, p) for p in phonemes] # Keep original if no map
        # return arpabet_phonemes
        # --- End Optional Conversion ---
...
# --- End of MyWERDetails class definition ---


# --- analyze_phoneme_differences function remains the same ---
...
def analyze_phoneme_differences(nl_wer_path: str, fr_wer_path: str, config_path: str) -> Optional[pd.DataFrame]:
    ...
    for phoneme in all_phonemes:
        ...
        if not phoneme or phoneme.isspace() or phoneme == ' ': # Check for empty string or space
            continue
        # Optional: Check if it looks like a valid phoneme (e.g., contains letters/numbers)
        # This regex might be too strict for some IPA symbols. Consider refining or removing.
        # if not re.search(r'[A-Z0-9]', phoneme, re.IGNORECASE):
        #      print(f"DEBUG: Skipping potential non-phoneme symbol: '{phoneme}'")
        #      continue
        ...
        if np.isnan(fr_error_rate) or np.isnan(nl_error_rate):
            # Decide how to handle: NaN, or treat missing PER as 0?
            # Let's use NaN for difference if either is NaN
            error_diff = np.nan
        else:
...
# --- create_frequency_vs_difference_plot function remains the same ---
...
def create_frequency_vs_difference_plot(df: pd.DataFrame, output_filename="phoneme_error_analysis.png"):
    ...
    plt.title('Phoneme Error Analysis: Frequency vs. Error Difference (FR vs NL Speakers)')
...
# --- create_important_phonemes_table function remains the same ---
...
def main():
    ...
    # --- Configuration ---
    nl_wer_path = r'experiments\jasmin_whisper\scoring_kaldi\wer_details_nl\per_utt_final_nl.txt'
    fr_wer_path = r'experiments\jasmin_whisper\scoring_kaldi\wer_details_fr\per_utt_final_fr.txt'
    config_path = r"configs\dutch.yaml"
    plot_output_path = r'phoneme_analysis_plot.png'
    # --- ADD CSV OUTPUT PATH ---
    csv_output_path = r'phoneme_analysis_full_results.csv'
