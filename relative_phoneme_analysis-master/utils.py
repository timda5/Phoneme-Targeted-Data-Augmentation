import yaml
from re import sub, compile # Added import for re module used later

def load_hparam(filename):
    # Use 'with open' for better file handling and add encoding
    try:
        with open(filename, 'r', encoding='utf-8') as stream:
            # Use yaml.safe_load or yaml.FullLoader
            # yaml.load_all returns a generator, so process it
            # Assuming the config is a single YAML document, yaml.safe_load is often preferred
            # If multiple documents are expected, stick with load_all and process the first one
            # Let's assume a single document for HParam based on typical usage
            # docs = yaml.load_all(stream, Loader=yaml.FullLoader) # Use FullLoader if complex types needed
            docs = yaml.safe_load(stream) # Use safe_load for standard types
            if docs is None:
                return {} # Return empty dict if file is empty
            # hparam_dict = dict()
            # for doc in docs: # If using load_all, you need to iterate
            #     if doc: # Check if the document is not empty
            #         for k, v in doc.items():
            #             hparam_dict[k] = v
            #     break # Assuming only the first document is the config
            # return hparam_dict
            return docs if isinstance(docs, dict) else {} # Return the loaded dict directly
        ...
        print(f"Error loading hparam file {filename}: {e}")
...
class Dotdict(dict):
    ...
    def __init__(self, dct=None):
        ...
        for key, value in dct.items():
            # Check if value is dictionary-like, but not Dotdict itself to prevent recursion issues
...
class HParam(Dotdict):
    ...
    def __init__(self, file):
        super().__init__() # Correct way to call parent __init__ for Dotdict
        hp_dict = load_hparam(file)
        # No need for hp_dotdict intermediate step if load_hparam returns a dict
        # The Dotdict __init__ handles recursive conversion
        # Update self with the loaded dictionary, Dotdict will handle nesting
    ...
    # These might not be necessary if inheriting directly from Dotdict
...
class CharConverter:
    ...
    def __init__(self, source, target):
        """
        Creates an instance of a character converter.
        Source is a string:  'uni', 'name', 'tipa', 'praat', 'xsampa',
        or 'latex'. Target is also a string, one of: 'uni','name','tipa','praat','xsampa',or 'latex'.
        ...
        if source == 'latex' or target == 'latex':
            ...
            if source == 'latex':
                ...
                for k, v in latex_charmap.items():
                    self.new_map[v[0]] = k
...
def tipaClean(s):
    ...
    tex_cmds = ['\^', '\\textupstep', '\!G', '\\textcorner', '\;L', '\|`', '\\textteshlig', '\|[',
                '\\textsuperscript{w}', '\|]', '\\textdoublepipe', '\\textramshorns', '\!g', '\c{c}', '\\textturnmrleg',
                '\\textvertline', '\'', '\;R', '\!j', '\:t', '\\textturnv', '\\textbeltl', '\:z', '\|>', '\!o', '\|\'',
                '\;H', '\\r*', '\\textdyoghlig', '\\textsuperscript{h}', '\|)', '\;N', '\\textdoublebarpipe',
                '\\textltailn', '\!b', '\!d', '\\textsuperscript{\\textgamma}', '\\textsuperscript{\\textrevglotstop}',
                '\\texththeng', '\o', '\:R', '\OE', '\`', '\=', '\\textbaru', '\"', '\*w', '\\textctz',
                '\\textdownstep', '\\textglobrise', '\\textdoublevertline', '\;B', '\|<', '\|(', '\\textbarglotstop',
                '\=*', '\"*', '\\v*', '\\textsuperscript{H}', '\~*', '\\textbardotlessj', '\:d', '\\textrhookschwa',
                '\\textcloserevepsilon', '\;G', '\\textlyoghlig', '\\textturnlonglegr', '\\textltilde', '\~', '\:s',
                '\H*', '\\textglobfall', '\|+', '\\textbarrevglotstop', '\\textcrh', '\:n', '\\textsuperscript{j}',
                '\\textpipe', '\oe', '\\textrhookrevepsilon', '\\textrhoticity', '\\ae', '\:l', '\*r', '\:r']
...
# Ensure the main execution block is safe if this file is imported elsewhere
if __name__ == '__main__':
    # converter = CharConverter("uni","tipa")
    # converter.convert(content) # 'content' is not defined, this line would error
